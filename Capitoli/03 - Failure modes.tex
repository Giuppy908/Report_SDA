...
La Failure Mode and Effects Analysis (FMEA) è una metodologia utilizzata per identificare le possibili modalità di guasto di un sistema e valutarne le conseguenze. L'analisi si concentra sugli asset ritenuti maggiormente significativi per il funzionamento della pipeline, a partire da quelli individuati nell'Asset Model. Per ciascun asset vengono esaminate le funzioni svolte, i relativi prerequisiti e i possibili failure mode, individuandone cause ed effetti sugli altri componenti del sistema.

I failure mode considerati possono derivare sia da condizioni accidentali sia da azioni intenzionali in grado di alterare il normale comportamento dell'asset. L'eventuale corrispondenza con specifiche minacce di sicurezza viene analizzata successivamente mediante STRIDE-AI.

\subsection*{Criteri di selezione degli asset per la FMEA}
\addcontentsline{toc}{subsection}{Criteri di selezione degli asset per la FMEA}
La FMEA viene applicata a un sottoinsieme degli asset individuati nel Capitolo 2, selezionati in base alla loro rilevanza nel flusso della pipeline, alla capacità di propagare failure verso altri componenti e al possibile impatto sulla decisione clinica o sui processi di aggiornamento.

Sulla base di tali criteri vengono approfonditi A01 - Immagine dermatologica, A13 - Feedback clinico validato, A16 - Modello EfficientNet-B4, A20 - Regole dell'Adaptation Layer e A21 - Piattaforma B4. Gli altri asset rimangono comunque considerati nel modello architetturale e
nelle dipendenze complessive del sistema.
...
L'asset A01 rappresenta l'immagine della lesione dermatologica acquisita per il caso clinico e utilizzata come principale informazione visiva della pipeline. L'immagine può essere recuperata tramite B4 oppure fornita direttamente attraverso l'endpoint \texttt{/analyze}.

All'interno della pipeline, viene utilizzata nello Stage 1 da EfficientNet-B4 per la classificazione iniziale dell'urgenza e nello Stage 2 da Qwen2-VL per la generazione della descrizione clinica della lesione. La correttezza e l'adeguatezza dell'immagine costituiscono quindi un prerequisito per le successive elaborazioni visive.

\subsubsection{Funzioni e prerequisiti}
Le principali funzioni dell'asset e i prerequisiti necessari al loro svolgimento sono riportati nella Tabella~\ref{tab:a01-functions}.
...
Protezione della riservatezza dell'immagine &

Accesso all'immagine consentito esclusivamente ai soggetti e ai componenti
autorizzati durante l'acquisizione, il trasferimento e l'elaborazione
...
Sulla base delle funzioni individuate vengono considerate le principali modalità con cui l'asset A01 può non svolgere correttamente il proprio compito, ovvero i failure mode.
...
rappresentativa della lesione effettiva. \\
...
FM01.6 \hypertarget{fm01-6}{} &

Esposizione non autorizzata dell'immagine dermatologica &

Accesso non autorizzato all'immagine durante il trasferimento o
l'elaborazione, configurazione errata dei controlli di accesso oppure
esposizione accidentale dell'input attraverso i componenti coinvolti
nel flusso di elaborazione. &

L'immagine dermatologica può essere resa accessibile a soggetti o
componenti non autorizzati, compromettendo la confidenzialità
dell'informazione clinica e la privacy del paziente. \\
...
L'asset A13 rappresenta il feedback clinico prodotto durante la revisione medica dei risultati generati da DermaTriage. Il feedback consente di confermare oppure correggere l'esito del sistema e viene associato al relativo caso clinico.

Le correzioni validate possono essere utilizzate nei meccanismi di aggiornamento della pipeline. In particolare, contribuiscono all'evoluzione dei prompt utilizzati dai modelli generativi e, nei casi
di disaccordo con la classificazione precedente, possono concorrere all'attivazione del retraining del classificatore EfficientNet-B4.

Il feedback clinico rappresenta quindi un collegamento diretto tra la revisione medica e il ciclo di aggiornamento del sistema.

\subsubsection{Funzioni e prerequisiti}
Le principali funzioni dell'asset e i prerequisiti necessari al loro svolgimento sono riportati nella Tabella~\ref{tab:a13-functions}.
...
\subsubsection{Failure mode}
Sulla base delle funzioni individuate vengono considerate le principali modalità con cui l'asset A13 può non svolgere correttamente il proprio compito, ovvero i failure mode.
...
\subsubsection{Funzioni e prerequisiti}
Le principali funzioni dell'asset e i prerequisiti necessari al loro svolgimento sono riportati nella Tabella~\ref{tab:a16-functions}.
...
\subsubsection{Failure mode}
Sulla base delle funzioni individuate vengono considerate le principali modalità con cui l'asset A16 può non svolgere correttamente il proprio compito, ovvero i failure mode.
...
L'asset A20 rappresenta l'insieme delle regole utilizzate dall'Adaptation Layer per convertire il livello di urgenza prodotto dalla pipeline nella scala clinica P1-P4 utilizzata dal sistema di
triage.

La conversione considera la classe di urgenza HIGH, MEDIUM o LOW e, per i casi classificati come HIGH, anche il livello di confidence. Un caso HIGH con confidence superiore a 0.85 viene associato alla
priorità P1, mentre gli altri casi HIGH vengono assegnati a P2. Le classi MEDIUM e LOW vengono convertite rispettivamente in P3 e P4.

L'Adaptation Layer produce inoltre le informazioni associate alla priorità finale, tra cui lo specialista indicato e il relativo SLA. Le regole di mapping costituiscono quindi il passaggio conclusivo tra il risultato interno della pipeline di inferenza e la rappresentazione
...
\subsubsection{Failure mode}
Sulla base delle funzioni individuate vengono considerate le principali modalità con cui l'asset A20 può non svolgere correttamente il proprio compito, ovvero i failure mode.
...
L'asset A21 rappresenta la piattaforma esterna B4, utilizzata come principale punto di interazione tra DermaTriage e il flusso clinico. Attraverso le API messe a disposizione dalla piattaforma, DermaTriage può recuperare i consulti e i relativi documenti, acquisire l'immagine dermatologica necessaria all'elaborazione e restituire il risultato diagnostico prodotto dalla pipeline.

B4 supporta inoltre la gestione delle validazioni mediche associate ai risultati di triage. Gli esiti clinici validati possono essere recuperati da DermaTriage e utilizzati nei meccanismi di aggiornamento del sistema,
tra cui l'evoluzione dei prompt e il retraining del classificatore EfficientNet-B4. L'accesso alle API B4 avviene mediante autenticazione basata su Bearer JWT.

\subsubsection{Funzioni e prerequisiti}
Le principali funzioni dell'asset e i prerequisiti necessari al loro svolgimento sono riportati nella Tabella~\ref{tab:a21-functions}.
...
%         F21.2 &
%         Messa a disposizione controllata dei dati necessari a DermaTriage &
%         Presenza sulla piattaforma delle informazioni e dei documenti
%         associati al consulto, disponibilità delle API necessarie al loro
%         recupero e accesso consentito esclusivamente attraverso i meccanismi
%         di autenticazione e autorizzazione previsti. \\
...
\subsubsection{Failure mode}
Sulla base delle funzioni individuate vengono considerate le principali modalità con cui l'asset A21 può non svolgere correttamente il proprio compito, ovvero i failure mode.
...
%         \hypertarget{fm21-5}{FM21.5} &
%         Esposizione non autorizzata dei dati clinici del consulto &
%         Configurazione errata dei controlli di accesso, compromissione o uso
%         improprio delle credenziali, dell'API key o del Bearer JWT, oppure
%         accesso alle informazioni del consulto da parte di soggetti o
%         componenti non autorizzati. &
%         Immagini dermatologiche, dati clinici e demografici, documenti,
%         risultati diagnostici o validazioni mediche possono essere resi
%         disponibili a soggetti non autorizzati, compromettendo direttamente
%         la confidenzialità delle informazioni e la privacy del paziente. \\
