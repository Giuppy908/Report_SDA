...
\begin{figure}[H]
    ...
    \node[external] (patient) {
        \textbf{Paziente}
    };

    \node[external, below=0.8cm of patient] (b4) {
        \textbf{B4}\\
        Piattaforma / Chatbot
    };

    % =====================================================
    % PIPELINE DERMATRIAGE
    % =====================================================

    \node[component, right=2.5cm of b4, yshift=1.0cm] (stage1) {
        \textbf{Stage 1}\\
        EfficientNet-B4\\
        \texttt{HIGH / MEDIUM / LOW}
    };

    \node[component, right=1.0cm of stage1] (stage2) {
        \textbf{Stage 2}\\
        Qwen2-VL\\
        Descrizione clinica
    };

    \node[component, below=1.0cm of stage2] (stage3) {
        \textbf{Stage 3}\\
        RAG + ChromaDB\\
        Casi clinici simili
    };

    \node[component, below=1.15cm of $(stage1)!0.5!(stage3)$] (stage4) {
        \textbf{Stage 4}\\
        BioMistral\\
        Sintesi clinica
    };

    \node[component, below=0.9cm of stage4] (adaptation) {
        \textbf{Adaptation Layer}\\
        Mapping verso P1--P4
    };
    ...
    \node[smallcomponent, below left=1.2cm and -0.2cm of adaptation] (prompt) {
        Evoluzione\\
        dei prompt
    };

    \node[smallcomponent, below right=1.2cm and -0.2cm of adaptation] (retraining) {
        Retraining\\
        EfficientNet-B4
    };

    % =====================================================
    % BOUNDARY DERMATRIAGE
    % =====================================================

    \begin{scope}[on background layer]
        \node[
            boundary,
            fit=(stage1)(stage2)(stage3)(stage4)(adaptation)(prompt)(retraining),
            label={[font=\bfseries]above:DermaTriage}
        ] (dermatriage) {};
    \end{scope}
    ...
    \draw[flow]
        (b4.east)
        -- ++(0.7,0)
        |- node[pos=0.76, above] {immagine}
        (stage1.west);

    \draw[flow]
        (b4.east)
        -- ++(1.0,0)
        |- node[pos=0.78, above] {immagine}
        (stage2.west);

    % Sintomi direttamente disponibili allo Stage 4
    \draw[flow]
        (b4.east)
        -- ++(0.55,0)
        |- node[pos=0.83, below] {sintomi}
        (stage4.west);
    ...
    % =====================================================
    % AGGIORNAMENTI -> INFERENZE FUTURE
    % =====================================================
    ...
    \caption{Architettura logica di DermaTriage e principali flussi di elaborazione.}
